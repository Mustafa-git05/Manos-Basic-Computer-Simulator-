# Manos-Basic-Computer-Simulator-
Cycle-accurate simulator for Mano’s Basic Computer. Loads programs and data from text files into simulated memory and supports step-by-step execution at cycle or instruction level. Fully models the CPU datapath, implements all Mano instructions, and includes CLI debugging and performance profiling (cycles, CPI, memory access).



Basic Manos Computer Simulator
...
EXECUTION COMMANDS:
- next_cycle Executes one cycle (T0, T1, ...)
- fast_cycle N Executes N cycles
...
3. Run all.
4. Proceed with the User I/O following the Execution and Inspection Menu.

TO RUN WITH VS CODE:
1. Change file path of program.txt and data.txt, ensuring both are in the same folder as the code itself.
2. Run with Python
3. Use the terminal for User I/O following the Execution and Inspection Menu.
